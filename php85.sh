...
# using Remi YUM repo's SCL php85 version
...
packages='php85 php85-php-fpm php85-php-devel php85-php-embedded php85-php-mysqlnd php85-php-bcmath php85-php-enchant php85-php-gd php85-php-pecl-geoip php85-php-gmp php85-php-pecl-igbinary php85-php-pecl-igbinary-devel php85-php-pecl-imagick-im6 php85-php-pecl-imagick-im6-devel php85-php-imap php85-php-intl php85-php-pecl-json-post php85-php-ldap php85-php-pecl-mailparse php85-php-mbstring php85-php-mcrypt php85-php-pecl-memcache php85-php-pecl-memcached php85-php-pecl-mysql php85-php-pdo-dblib php85-php-pspell php85-php-pecl-redis5 php85-php-snmp php85-php-soap php85-php-tidy php85-php-xml php85-php-xmlrpc php85-php-pecl-zip php85-php-opcache php85-php-sodium libsodium-devel oniguruma5php oniguruma5php-devel php85-php-brotli php85-php-zstd php85-php-zstd-devel php85-php-process'
...
phpsededit() {
    ...
    CONFIGSCANDIR='/etc/opt/remi/php85/php.d'
    ...
    opcachehugepages
    if [ -f /etc/opt/remi/php85/php-fpm.d/www.conf ]; then
      sed -i 's|pm.max_children = .*|pm.max_children = 20|' /etc/opt/remi/php85/php-fpm.d/www.conf
      sed -i 's|pm.max_spare_servers = .*|pm.max_spare_servers = 15|' /etc/opt/remi/php85/php-fpm.d/www.conf
    fi
}
...
phpinstall() {
  ...
  phpsededit
  if [ ! -f /var/opt/remi/php85/log/php-fpm/www-error.log ]; then
    touch /var/opt/remi/php85/log/php-fpm/www-error.log
    chmod 0666 /var/opt/remi/php85/log/php-fpm/www-error.log
    chown nginx:nginx /var/opt/remi/php85/log/php-fpm/www-error.log
  fi
  if [ ! -f /var/opt/remi/php85/log/php-fpm/www-slow.log ]; then
    touch /var/opt/remi/php85/log/php-fpm/www-slow.log
    chmod 0666 /var/opt/remi/php85/log/php-fpm/www-slow.log
    chown nginx:nginx /var/opt/remi/php85/log/php-fpm/www-slow.log
  fi
  echo "systemctl stop php85-php-fpm" >/usr/bin/fpm84stop ; chmod 700 /usr/bin/fpm84stop
  echo "systemctl start php85-php-fpm" >/usr/bin/fpm84start ; chmod 700 /usr/bin/fpm84start
  echo "systemctl restart php85-php-fpm" >/usr/bin/fpm84restart ; chmod 700 /usr/bin/fpm84restart
  echo "systemctl reload php85-php-fpm" >/usr/bin/fpm84reload ; chmod 700 /usr/bin/fpm84reload
  echo "systemctl status php85-php-fpm" >/usr/bin/fpm84status ; chmod 700 /usr/bin/fpm84status
  echo "nano -w /etc/opt/remi/php85/php-fpm.d/www.conf" >/usr/bin/fpmconfphp85 ; chmod 700 /usr/bin/fpmconfphp85
  echo "nano -w /usr/local/nginx/conf/php85-remi.conf" >/usr/bin/phpincphp85 ; chmod 700 /usr/bin/phpincphp85    
  sed "s|fastcgi_param  SERVER_NAME        \$server_name;|fastcgi_param  SERVER_NAME        \$http_host;|" /usr/local/nginx/conf/php.conf > /usr/local/nginx/conf/php85-remi.conf
  rsync -ah /bigscoots/wpo/phpfpm/templates/php-version-pool.conf.template /etc/opt/remi/php85/php-fpm.d/www.conf
  sed -i 's|PHPVERSION|php85|' /etc/opt/remi/php85/php-fpm.d/www.conf
  sed -i 's|\[www\]|\[php85-www\]|' /etc/opt/remi/php85/php-fpm.d/www.conf
  sed -i 's|9000|24000|' /etc/opt/remi/php85/php-fpm.d/www.conf
  sed -i 's|9000|24000|' /usr/local/nginx/conf/php85-remi.conf
  sed -i 's/fastcgi_pass dft_php;/#fastcgi_pass dft_php;/g' /usr/local/nginx/conf/php85-remi.conf
  sed -i 's/#fastcgi_pass   127.0.0.1/fastcgi_pass   127.0.0.1/g' /usr/local/nginx/conf/php85-remi.conf
  sed -i 's|;listen.backlog = .*|;listen.backlog = 511|' /etc/opt/remi/php85/php-fpm.d/www.conf
  sed -i 's|;listen.owner = .*|listen.owner = nginx|' /etc/opt/remi/php85/php-fpm.d/www.conf
  sed -i 's|;listen.group = .*|listen.group = nginx|' /etc/opt/remi/php85/php-fpm.d/www.conf
  sed -i 's|user = apache|user = nginx|' /etc/opt/remi/php85/php-fpm.d/www.conf
  sed -i 's|group = apache|group = nginx|' /etc/opt/remi/php85/php-fpm.d/www.conf
  sed -i 's|;pm.status_path = \/status|pm.status_path = \/php85status|' /etc/opt/remi/php85/php-fpm.d/www.conf

  # raise system limits
  mkdir -p /etc/systemd/system/php85-php-fpm.service.d
  echo -en "[Service]\nLimitNOFILE=262144\nLimitNPROC=16384\n" > /etc/systemd/system/php85-php-fpm.service.d/limit.conf
  systemctl daemon-reload

  echo
  echo "start php85-php-fpm service"
  systemctl start php85-php-fpm
  systemctl enable php85-php-fpm
  echo
  echo "status php85-php-fpm service"
  systemctl status php85-php-fpm
  echo
  echo "command shortcuts"
  echo "phpincphp85 - edit /usr/local/nginx/conf/php85-remi.conf include file"
  echo "fpmconfphp85 - edit /etc/opt/remi/php85/php-fpm.d/www.conf php-fpm config"
  echo
  echo "php85 -v"
  php85 -v
  echo
...
case "$1" in
  ...
  list )
    ...
    ;;
  phpconfig )
    if [ -f /opt/remi/php85/root/usr/bin/php-config ]; then
      /opt/remi/php85/root/usr/bin/php-config
    fi
    ;;
  phperrors )
    if [ -f /var/opt/remi/php85/log/php-fpm/www-error.log ]; then
      echo "tail -100 /var/opt/remi/php85/log/php-fpm/www-error.log"
      tail -100 /var/opt/remi/php85/log/php-fpm/www-error.log
    fi
    ;;
  phpcustom )
    if [ -f /etc/opt/remi/php85/php.d/zzz_customphp.ini ]; then
      nano /etc/opt/remi/php85/php.d/zzz_customphp.ini
    fi
    ;;
  phpslowlog )
    if [ -f /var/opt/remi/php85/log/php-fpm/www-slow.log ]; then
      echo "tail -100 /var/opt/remi/php85/log/php-fpm/www-slow.log"
      tail -100 /var/opt/remi/php85/log/php-fpm/www-slow.log
    fi
    ;;
  phpini )
    php85 --ini
